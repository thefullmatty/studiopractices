...
// Mathematical 150 cm eyeline based on the drawn wall scale.
// This replaces the static exported eyeline SVG so scale and eyeline agree.
const EYE_LEVEL_CM = 150;
const EYE_LINE_Y = WALL.y + WALL.h - EYE_LEVEL_CM * PX_PER_CM;
...
function exportPlanner() {
  // Compose the vector SVG layers and generated artworks into one high-resolution PNG.
  // The HTML input values are not exported, because the SVG already contains the field/table design.
  const exportScale = 3;
  const out = document.createElement('canvas');
  out.width = Math.round(DESIGN_W * exportScale);
  out.height = Math.round(DESIGN_H * exportScale);

  const ctx = out.getContext('2d');
  ctx.scale(exportScale, exportScale);
  ctx.clearRect(0, 0, DESIGN_W, DESIGN_H);

  const gallery = document.getElementById('layer-gallery');
  const sections = document.getElementById('layer-sections');
  const textLayer = document.getElementById('layer-text');

  ctx.drawImage(gallery, 0, 0, DESIGN_W, DESIGN_H);
  ctx.drawImage(sections, 0, 0, DESIGN_W, DESIGN_H);
  ctx.drawImage(textLayer, 0, 0, DESIGN_W, DESIGN_H);
  drawArtworksToContext(ctx);
  drawMathematicalEyeLineToContext(ctx);

  out.toBlob((blob) => {
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = '2d-wall-installation-planner.png';
...
function drawMathematicalEyeLine() {
  push();
  const red = color(255, 38, 38);
  stroke(red);
  fill(red);
  strokeWeight(1.3);

  // Dashed horizontal eye line across the wall.
  drawingContext.setLineDash([5, 4]);
  line(WALL.x, EYE_LINE_Y, WALL.x + WALL.w, EYE_LINE_Y);
  drawingContext.setLineDash([]);

  // Vertical measuring arrow from floor to eye line.
  const arrowX = WALL.x + WALL.w - 10;
  const floorY = WALL.y + WALL.h;
  line(arrowX, floorY, arrowX, EYE_LINE_Y);
  drawArrowHead(arrowX, EYE_LINE_Y, 'up');
  drawArrowHead(arrowX, floorY, 'down');

  noStroke();
  textAlign(CENTER, CENTER);
  textSize(11);
  text('1500mm', arrowX - 32, EYE_LINE_Y + (floorY - EYE_LINE_Y) / 2);
  pop();
}
...
function drawMathematicalEyeLineToContext(ctx) {
  ctx.save();
  const red = '#ff2626';
  ctx.strokeStyle = red;
  ctx.fillStyle = red;
  ...
  ctx.setLineDash([5, 4]);
  ctx.beginPath();
  ctx.moveTo(WALL.x, EYE_LINE_Y);
  ctx.lineTo(WALL.x + WALL.w, EYE_LINE_Y);
  ctx.stroke();
  ctx.setLineDash([]);
  ...
  const floorY = WALL.y + WALL.h;
  ctx.beginPath();
  ctx.moveTo(arrowX, floorY);
  ctx.lineTo(arrowX, EYE_LINE_Y);
  ctx.stroke();

  drawArrowHeadToContext(ctx, arrowX, EYE_LINE_Y, 'up');
  drawArrowHeadToContext(ctx, arrowX, floorY, 'down');

  ctx.font = '11px Arial, Helvetica, sans-serif';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('1500mm', arrowX - 32, EYE_LINE_Y + (floorY - EYE_LINE_Y) / 2);
  ctx.restore();
}

function drawArrowHeadToContext(ctx, x, y, direction) {
  const s = 7;
  ctx.beginPath();
  if (direction === 'up') {
    ctx.moveTo(x, y);
...
function drawArtworksToContext(ctx) {
  artworks.forEach((artwork) => {
    ctx.save();
    ...
    ctx.lineWidth = 1.2;
    ctx.beginPath();
    ctx.rect(artwork.x, artwork.y, artwork.w, artwork.h);
    ctx.fill();
    ctx.stroke();
    ...
    const centreX = artwork.x + artwork.w / 2;
    const centreY = artwork.y + artwork.h / 2;
    ctx.fillText(`Artwork ${artwork.label}`, centreX, centreY - 7);
    ctx.fillText(`${formatCm(artwork.originalWidthCm)} × ${formatCm(artwork.originalHeightCm)} cm`, centreX, centreY + 7);
    ctx.restore();
  });
}
